// memload.cpp : Defines the entry point for the console application.
//
...
int _tmain(int argc, char *argv[]){
	...
	arg0 = strtok_s(cmdline, " ", &nextarg);
	arg1 = strtok_s(NULL, " ", &nextarg);
	arg2 = strtok_s(NULL, " ", &nextarg);
	...
	warning_threshold = atoi(arg1);
	critical_threshold = atoi(arg2);
	...
	printf("Mem: %I64d MB (%d%%) / %I64d MB (%d%%) ",
	...
	* So dont print out anything if its fscked */
	if (mean_pused >= 0 && mean_pused <= 100) {
		printf("Paged Mem: %I64d MB (%d%%) / %I64d MB (%d%%)\n",
	...
	if ((unsigned int)floor(mean_used) >= critical_threshold)
		return Critical;

	if ((unsigned int)floor(mean_used) >= warning_threshold)
		return Warning;
